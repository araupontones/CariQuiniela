...
indicators_matches <- function(.data, prefix, 
  ...
  data_venue <- .data %>% filter(!is.na({{goles_favor}}),Venue %in% venues)
  ...
  summarise("matches" := n(),
            ...
            #did score ---------------------------------------------------
            did_score = sum({{goles_favor}} >0),
            #did_score_index = ({{goles_favor}} > 0) * visitante_fifa_index,
            
            #fail to score --------------------------------------------------
            "fail_to_score" := sum({{goles_favor}} == 0),
            #fail_to_score_index = ({{goles_favor}} == 0) * (1 - visitante_fifa_index),
            
            #did receive goal -----------------------------------------------
            did_receive_goal = sum({{goles_contra}} >0),
            #did_receive_goal_index = ({{goles_contra}} > 0) * visitante_fifa_index,
            
            #didnt receive goal ---------------------------------------------
            didnt_receive_goal = sum({{goles_contra}} == 0),
            #didnt_receive_goal_index = ({{goles_contra}} == 0) * (1 - visitante_fifa_index),
            ...
            "Gdiff" := {{goles_favor}} - {{goles_contra}},
            .groups = 'drop'
  ) %>%
